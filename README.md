...
This project provides a study of U.S. court outcomes and opinions about fair use and copyright
civil cases. It analyzes a dataset containing various features related to court cases 
with text data, numerical data, and categorical data. In addition, it builds and evaluates
machine learning models to determine their predictive performance using two sets of features: 
one with case opinions (w_op) and one without case opinions (wo_op).  The target variable is the case 
outcome which is based on the case opinion.<br>
...
significant improvements with text data, indicating their ability to effectively utilize additional information.<br><br>
The Random Forest Classifier with
hyperparameter tuning is the best model for the set without the case opinion because the GridSearch with 
RandomForestClassifier_wo_op version showed a more balanced performance, 
suggesting that hyperparameter tuning effectively managed overfitting. This
indicates that it generalizes well to unseen data and effectively balances precision and recall, highlighting the 
importance of optimizing model parameters. The trade-off between computational cost (fit time) and model performance 
should be considered when deploying these models in practice.
<br><br>

# 1. Business understanding 
The U.S. Copyright Office defines copyright [https://www.copyright.gov/what-is-copyright/]  as a <br>
<i>
"type of intellectual property that protects original works of authorship as soon as an author fixes the work in a tangible form of expression. In copyright law, there are a lot of different types of works, including paintings, photographs, illustrations, musical compositions, sound recordings, computer programs, books, poems, blog posts, movies, architectural works, plays,"
</i><br>
etc.. In addition, copyright infrigement involves the unauthorized use or reproduction, distribution, public display, creation or derivative of copyrighted works. <br>
<br>
...
## Overview
This analysis evaluates the predictive power of various machine learning models on a subset of civil cases, and focus on 
the litigation results of copyright infringement found (or not found) and fair use found (or not found).<br>
...
It demonstrates that most of the cases in this dataset are in SCOTUS, then the 2nd Circuit (New York City's lower Manhattan), and 
...
<br><br>
Finally, we look at a scatter plot of court and jurisdiction with outcome as a hue to identify that SCOTUS has a 
larger number of pending cases. In addition, most courts have cases unassigned to a jurisdiction.
<br>
...
## Methodology
1. Identify the models to train and test with. 
2. Use GridSearch for hyperparameter tuning with the aim to identify the best-performing algorithm for predicting
legal case outcomes based on the given features.
3. Create two different sets of data. 
...
4. Create two different preprocessors.
...
5. Train and test each of the models without case opinions. 
6. Train and test each of the models with case opinions.
<br>

## Identified Model Types
* RandomForestClassifier: a powerful ensemble method that combines the predictions of multiple decision trees to improve generalization and reduce overfitting. <br>
* LogisticRegression: a simple yet effective linear model and as a baseline model due to its fast training times<br>
* KNN: can be effective for smaller datasets and can capture non-linear relationships without the need for explicit model training.<br>
* Gradient Boosting: builds models sequentially, with each new model attempting to correct the errors of the previous ones.<br>
* SVM: versatile and robust model that can perform both linear and non-linear classification
<br>

## Model Training times and testing metrics including GridSearch

![results.png](images%2Fresults.png)
<br>
Bellow we visualize both the fit time (in seconds) and the test accuracy for each 
model. The dark green bars represent the fit time for each model, with text 
labels on top showing the values obtained. The cornflower blue line with markers 
represents the test accuracy for each model. Including case opinions results in higher fit 
...
green bars represent the fit time for each model, with text labels on top showing the 
values obtained. The cornflower blue line with markers represents the F1 score for each model. 
Including case opinions results in higher fit times but with improved F1 scores for several models, 
...
The GridSearch improved model performance, particularly for Gradient Boosting and SVM models. 
